...
short gameFunc_Win (struct player *playerA, struct player *playerB);            //CHEAT - Makes a player instantly win a game
...
int main (void) {
  ...
  //system("mode 700");
	...
	while (1) {
		...
		unsigned char _user[strlen(user) + 1];

		//Removes case sensitivity
		for (k = 0, _user[strlen(user)] = '\0'; k < strlen(user); k++)
			if (user[k] >= 'A' && user[k] <= 'Z') _user[k] = user[k] + 32;
			else _user[k] = user[k];

		if (!strcmp (_user, "ajuda")) {
			recognized = 1;
			gameFunc_Ajuda (NULL);
		}
		else if (!strcmp (_user, "iniciar")) break;
		else recognized = 0;
    ...
    main_feedback = SUCCESS; //main_feedback starts as SUCCESS by default
		...
		while (main_feedback != RESET && player[!turn].remaining[0]) {
...
short aux_getFunction (unsigned char str[]) {
	...
	for (k = 0, strBuf[strlen(str)] = '\0'; k < strlen(str); k++) //Removes case sensitivity
		if (str[k] >= 'A' && str[k] <= 'Z') strBuf[k] = str[k] + 32;
		else strBuf[k] = str[k];
...
short gameMain (struct player *curPlayer, struct player *stdbyPlayer) {
    ...
    case 8 : return gameFunc_Win (curPlayer, stdbyPlayer);      break;
...
short gameFunc_Gravar (struct player *playerA, struct player *playerB) { //timestamp !!//
...
short gameFunc_Win (struct player *playerA, struct player *playerB) {
...
void  printLogo (int typeLogo) {
	...
	short k, i, j, space_feed; //Counting variabels
	char cBuffer; //Buffer
	...
	const long long btScr[5][5] = { //Texts generation seeds
	  ...
	  {0x3C0FC3FC3C0000, 0xC303C3C3C30000, 0xC303C3C3C30000, 0xFFC3C3C3FF0000, 0xC33C3CFCC30000},
		{0xFFFFC30FCFF000, 0xC018E70C3C0000, 0xFC18FF0C3FF000, 0xC018DB0C3C0000, 0xC0FFC30FCFF000},
		{0x00F3C3F3C00000, 0x003C3C0C300000, 0x003C3CFC300000, 0x0C3C3C3C300000, 0x03C3C3C3C00000}};

	//BATALHA NAVAL
  int data_line, spaceID;
	const char spaces[5][7] = {{55, 47, 39, 31, 23, 15, 7}, {55, 47, 39, 31, 23, 15, 7},
	                           {55, 47, 39, 31, 23, 15},
		    									   {55, 47, 39, 27, 19}, {55, 52, 43, 35, 27, 19}};
