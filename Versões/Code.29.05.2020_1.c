...
int main (void) {
  ...
  //system("mode 700");
	...
	while (1) {
		...
		if (!strcmp (user, "ajuda")) {
			recognized = 1;
			gameFunc_Ajuda (NULL);
		}

		else if (!strcmp (user, "iniciar")) break;
...
short aux_getFunction (char *str) {
	short k;
	const char functionTitle[][10] = {"pow", "ajuda", "reset", "sair", "acaso", "gravar", "carregar", "revelar", "win"};

	aux_lowerCaseOnly (str);

	for (k = 0; k < 9 && strcmp (str, functionTitle[k]); k++);
...
void  printLogo (int typeLogo) {
	...
	const long long btScr[5][5] = { //Texts generation seeds
	  {0xFC3CFF3CC0C33C, 0xC3C318C3C0C3C3, 0xFCC318C3C0FFC3, 0xC3FF18FFC0C3FF, 0xFCC318C3FFC3C3},  //BATALHA
	  {0x00C33CC33CC000, 0x00F3C3C3C3C000, 0x00CFC3C3C3C000, 0x00C3FF66FFC000, 0x00C3C318C3FF00},  //NAVAL
	  {0x3C0FC3FC3C0000, 0xC303C3C3C30000, 0xC303C3C3C30000, 0xFFC3C3C3FF0000, 0xC33C3CFCC30000},  //AJUDA
		{0xFFFFC30FCFF000, 0xC018E70C3C0000, 0xFC18FF0C3FF000, 0xC018DB0C3C0000, 0xC0FFC30FCFF000},  //FIM DE
		{0x00F3C3F3C00000, 0x003C3C0C300000, 0x003C3CFC300000, 0x0C3C3C3C300000, 0x03C3C3C3C00000}}; //JOGO
