...
struct player {
		...
		int score;
		unsigned id;
		unsigned remaining; //left to win
};
...
short gameFunc_Pow (struct player *curPlayer) {
		...
		printf ("\n    Pressione qualquer tecla para finalizar a jogada...");
		fflush(stdin);
		getchar();
...
void printMatch (struct player *curPlayer, char showHiddenShips) {
		...
		for (i = 0; i < SIZE; i++) { //To each row i
			...
			for (j = 0; j < SIZE; j++) {
				...
				char buffer1 = curPlayer->match[i][j];
				char buffer0 = EMPTY_CELL;

				if (buffer1 >= 'a' && buffer1 <= 'z' && showHiddenShips == 0)
					buffer1 = EMPTY_CELL;

				if (buffer1 >= 'A' && buffer1 <= 'Z')
					buffer0 = (curPlayer->id ? 253 : 251);
...
void printBootScreen () {
		...
		printf ("\n\nPressione qualquer tecla para prosseguir ou digite \"AJUDA\" para informa%c%ces\n", 135, 228); //carregar etc
		getchar(); //Waits for
}
