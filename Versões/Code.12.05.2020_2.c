...
short gameMain (struct player *curPlayer);
...
void gameFunc_Help (void);
short gameFunc_Sair (void);
short gameFunc_Salvar (struct player *curPlayer, char *str)
short gameFunc_Abrir (struct player *curPlayer);
...
int main (void) {
		...
		while (player1.remaining > 0 && player2.remaining > 0) {
			...
			while (gameMain (&player1));

			printMatch (&player2, SHOW_HIDDEN);
			while (gameMain (&player2));
		}
...
void print (struct player *curPlayer) {
	...
	printf ("\n\n%ld %d %d %d %d %d", curPlayer->time, curPlayer->score, curPlayer->score,
										curPlayer->id, curPlayer->remaining, curPlayer->shots);
...
short gameMain (struct player *curPlayer) { //all game functions
		...
		if (!strcmp(func, "pow")) {
			//returns:  0 to success, 1 to invalid input
			if (gameFunc_Pow (curPlayer)) {
				printf ("     -Entrada inv%clida!", 160);
				return ERROR;
			}
		}
		else if (!strcmp(func, "sair")) {
				return gameFunc_Sair();
		}
		else if (!strcmp(func, "reset")) {
			return 1;
		}
	  else if (!strcmp(func, "ajuda")) {
		  gameFunc_Help ();
			printMatch (curPlayer, SHOW_HIDDEN);
			return ERROR;
	  }
		else if (!strcmp(func, "acaso")) {
		    return 1;
	  }
		else if (!strcmp(func, "gravar")) {
	      char str[30];
				gets(str);
				gameFunc_Salvar(curPlayer, str);
	  }
		else if (!strcmp(func, "carregar")) {
		    return 1;
		}
		else {
...
short gameFunc_Pow (struct player *curPlayer) {
		...
		else                               return  1; //Command syntax not recognized
		...
		printf ("\n    Pressione ENTER para finalizar a jogada...");
		fflush(stdin);
		getchar();
...
short gameFunc_Salvar (struct player *curPlayer, char *str) {
	...
	fwrite (curPlayer, sizeof(struct player), 1, arq);
	fprintf (arq, ";");
...
short gameFunc_Abrir (struct player *curPlayer) {
	...
	if (arq == NULL)
		printf ("deu erro aqui poh");

	fread (curPlayer, sizeof(struct player), 1, arq);
