...
void  aux_kbBuffer_clear (void);                                                //Implements a general way to clear the keyboard buffer
...
int main (void) {
  ...
  //system("mode 700");
	...
	while (1) {
		...
		player[0] = fillStruct (1);
		player[1] = fillStruct (2);

		//main_feedback, will receive the returned value by game_main witch will then be interpreted
		//turn, oscillates between 0 and 1, defining which player plays on that round
		short main_feedback = 1, turn = 0;

		//By default, a match always starts with hidden ships
		show_ships = 0;

		//Sets timer0 as the current time, right before the match stars, it'll be used as a time reference
		timer0 = clock();

		//A match continues while the opponent of the current player haven't won and if noboby asked for reset
		while (player[!turn].remaining[0] > 0 && main_feedback != RESET) {

			//The board is first printed on the screen
			printMatch (&player[turn]);
			...
			//At the end, the turn switches, meaning is time to the other player to play
			turn = !turn;
		}

		//if the match was ended by a reset, it starts again going to the main loop
		if (main_feedback == RESET) continue;

		//if the match ended because one of the players actually won, the main loop is broken
		else break;
	}
...
short aux_getFunction (unsigned char str[]) {
	...
	const unsigned char functionTitle[][10] = {"pow", "ajuda", "reset", "sair", "acaso", "gravar", "carregar", "revelar", "win"};
	...
	for (k = 0; k < 9 && strcmp (strBuf, functionTitle[k]); k++);

	return k;
}

void  aux_kbBuffer_clear (void) {
	//Clears the keyboard buffer
	scanf ("%*[^\n]"); //Reads a char string untill find the char '\n'
	scanf ("%*c"); //Reads the '\n' char
}
	...
	do {
		...
		switch (inp) {
			...
			case '2' : {
				...
				printf("  -AJUDA:    Concede informa%c%ces ao jogador a qualquer momento da partida", 135, 228);
...
short gameFunc_Carregar (struct player *playerA, struct player *playerB) {
	...
	if (arq == NULL)
		printf ("     -Nao foi poss%cvel encontrar o arquivo...\n", 161);
...
short gameFunc_Reset (struct player *playerA, struct player *playerB) {
    ...
    if (aux_optionMenu("Deseja realmente resetar o jogo?")){
		   ...
		   //if (aux_optionMenu("Deseja salvar o jogo?"))
			 //	 gameFunc_Gravar(playerA, playerB);
