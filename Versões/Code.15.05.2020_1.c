...
short gameFunc_Ajuda (struct player *playerA, struct player *playerB);
...
struct player fillStruct (int id) {
	...
	unsigned k, content[5] = {84, 11, 20, 21, 32};
	for (k = 0; k < 5; k++) buffer.remaining[k] = content[k];
...
short gameMain (struct player *curPlayer, struct player *stdbyPlayer) { //all game functions
	...
	switch (aux_getFunction(func)) {
		...
		case 1 : return gameFunc_Ajuda (curPlayer, stdbyPlayer);  break;
...
short gameFunc_Pow (struct player *curPlayer) {
	...
	unsigned scoreMod;											  //Variable that will store the score modifier
	...
	if (buffer >= 'a' && buffer <= 'z') {     //if the position contains a ship
   	...
   	curPlayer->remaining[0] -= 1;           //uptades remaining shots to player to win
		...
		switch (curPlayer->match[row][col]) {
		...
		//Score decrement
		scoreMod = -5 * (float)(1.0 - curPlayer->remaining[0]/(SIZE * SIZE));

		//Satures player score at minimum 0
		if ((int)((curPlayer->score) - scoreMod) < 0)
			scoreMod = 0;
	}
...
short gameFunc_Ajuda (struct player *playerA, struct player *playerB) {
	...
	system ("cls");
	char inp;
	printf("Digite o n%cmero corresponde a uma das op%c%ces:\n1-Jogo\n2-Comandos\n3-Navios\n4-Sair\n\n",163 ,135, 228);
	fflush(stdin);
	scanf("%c", &inp);
	...
	switch (inp) {
		...
		case 2 : {
		printf("\n-Pow: faz o bombardeio em uma posi. Ex: pow 2H.\n");
	 	...
	 	printf("-Sair: fecha o jogo.\n");
	 	printf("-Carregar: recarrega uma partida salva.\n");
	 	printf("-Reset: inicia uma nova partida, apagando todos os dados da atual.\n");
		printf("-Revelar: revela ou oculta as posições de todas as embarca%c%ces.", 135, 228);
		waitForEnter ("voltar...");
		break;
		}

		case 3 :{ //prints all the ships
		...
		case 4 : waitForEnter ("sair..."); break;// sair
	}

	printMatch (playerA, playerB);
	return 1;
}
